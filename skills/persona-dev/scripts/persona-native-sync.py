...
def http_url_credential_free(url: str, name: str) -> None:
    parsed = urlsplit(url)
    if parsed.username is not None or parsed.password is not None:
        fail(f"MCP {name} HTTP url must not contain credentials")
    for parameters in (parsed.query, parsed.fragment):
        if any(SENSITIVE.search(key) for key, _ in parse_qsl(parameters, keep_blank_values=True)):
...
def selected_codex_mcps(servers: dict[str, object], names: list[str]) -> dict[str, dict[str, object]]:
    ...
    for name in names:
        ...
        if kind == "stdio":
            ...
            http_url_credential_free(raw["url"], name)
