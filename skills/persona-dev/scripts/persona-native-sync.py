...
SENSITIVE = re.compile(r"(?:^|[_-])(?:api[_-]?key|token|secret|password|authorization|cookie)$", re.I)
...
def credential_free(value: object, key: str = "") -> None:
    if SENSITIVE.search(key) and not (isinstance(value, str) and re.fullmatch(r"\$\{[A-Za-z_][A-Za-z0-9_]*\}", value)):
